...
function del(id){
	if(confirm("确认删除?")){
		location.href='manager/deleteBook?id='+id;
	}
}
</script>
<form action="manager/findBookByTag" method="post">
...
<div class="layui-form layui-border-box layui-table-view" >
	...
	<a href="manager/toUpdateBook?id=${row.id}">修改</a>
	</td></tr>

	</c:forEach>
	<c:if test="${sessionScope.selectAct.equals('all')}">
	<tr>
		<td>
			&nbsp;&nbsp;&nbsp;
			&nbsp;&nbsp;&nbsp;共${totalCount}条记录&nbsp;&nbsp;共${totalPage}页&nbsp;&nbsp;
			第${pageCur}页&nbsp;&nbsp;
			<c:url var="url_pre" value="manager/allBooks">
				<c:param name="pageCur" value="${pageCur - 1 }"/>
			</c:url>
			<c:url var="url_next" value="manager/allBooks">
				<c:param name="pageCur" value="${pageCur + 1 }"/>
			</c:url>
			<!-- 第一页没有上一页 -->
			<c:if test="${pageCur != 1 }">
