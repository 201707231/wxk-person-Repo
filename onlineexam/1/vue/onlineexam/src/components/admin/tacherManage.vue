...
<template>
  <div class="all">
    <ul class="top">
      <li class="search-li"><div class="icon"><input type="text" placeholder="教师名称" class="search" v-model="key"><i class="el-icon-search"></i></div></li>
      <li><el-button type="primary" @click="search()">搜索教师</el-button></li>
      <li><el-button type="primary" @click="getTeacherInfo()">查询全部</el-button></li>
    </ul>
    <el-table :data="pagination.records" border>
      ...
      <el-table-column fixed="right" label="操作" width="150">
        <template slot-scope="scope">
          <a @click="checkGrade(scope.row.teacherId)" >编辑</a>
          <a @click="deleteById(scope.row.teacherId)">删除</a>
        </template>
      </el-table-column>
    </el-table>
    <el-pagination
...
export default {
  ...
  methods: {
    ...
    handleClose(done) { //关闭提醒
      ...
      search() {
          this.$axios('/api/teachers').then(res => {
              if(res.data.code == 200) {
                  let allTeacher = res.data.data
                  let newPage = allTeacher.filter(item => {
                      return item.teacherName.includes(this.key)
                  })
